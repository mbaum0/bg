 ...
 * In the future `BgConfig` can also include information about Crawford, cube possession, strength of the engine and so
 * on.
 */
 ...
 */
struct CMove best_move(const struct Wildbg* wildbg, const int (*pips)[26], unsigned int die1, unsigned int die2,
 ...
 * Checkers of the player on turn are encoded with positive integers, the opponent's checkers with negative integers.
 */
struct CProbabilities probabilities(const struct Wildbg* wildbg, const int (*pips)[26]);
